...
def parse_wheel(wheel_zip, name):
    ...
    Returns the name of the .static-info directory and the parsed WHEEL metadata.
    """
    try:
...
def wheel_dist_info_dir(source, name):
    ...
    """Returns the name of the contained .static-info directory.
    ...
    info_dirs = [s for s in subdirs if s.endswith('.static-info')]

    if not info_dirs:
        raise UnsupportedWheel(".static-info directory not found")

    if len(info_dirs) > 1:
        raise UnsupportedWheel(
            "multiple .static-info directories found: {}".format(
                ", ".join(info_dirs)
            )
        )
    ...
    if not info_dir_name.startswith(canonical_name):
        raise UnsupportedWheel(
            ".static-info directory {!r} does not start with {!r}".format(
